...
public:
    ...
    struct Bounds {
        float x_min_ = std::numeric_limits<float>::min();
        float x_max_ = std::numeric_limits<float>::max();
        float y_min_ = std::numeric_limits<float>::min();
    ...
    struct Node {
        ...
        bool is_bucket() const { return bucket_start_ != -1; }
    };
    ...
    double get_coordinate(const Point &p, int8_t dim) const {
        assert(dim == 0 || dim == 1);
        return (dim == 0) ? p.first : p.second;
    }


    void select(int lower, int upper, int middle, int8_t dim) {
        auto l = static_cast<size_t>(lower);
        auto u = static_cast<size_t>(upper);
        auto m = static_cast<size_t>(middle);

        std::nth_element(
                bucket_points_.begin() + l,
                bucket_points_.begin() + m,
                bucket_points_.begin() + u + 1,
    ...
    double get_distance(size_t first_pt_idx, size_t second_pt_idx) const {
    ...
    bool ball_in_bounds(const Bounds &bounds, uint32_t point_idx,
                        double radius) const {
        assert(radius >= 0);
    ...
    void fixed_radius_nn(uint32_t point_idx, double radius,
